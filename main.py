...
for db in dbs:
    ...
    sql = """SELECT DATE_FORMAT(created_at, '%%W %%d, %%M %%Y'), COUNT(*) AS number_of_records FROM properties GROUP BY DATE_FORMAT(created_at, '%%W %%d, %%M %%Y') WHERE DATE(created_at) = CURDATE()"""
